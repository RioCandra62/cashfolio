...
import { getSavedByID, tabungById } from "@/lib/saving";

export default function EditTabung({
  savingId,
  onClose,
}: {
  savingId: string;
  ...
  const [savingDetails, setSavingDetails] = useState<any>(null);

  async function getSavingDetails(savingId: string) {
    const res = await getSavedByID(savingId);
    setSavingDetails(res);
  }
      ...
      >
        ...
        <div className="p-6 border-b flex justify-between items-center">
          <div>
            ...
            <p className="text-sm text-gray-500">Masukkan nominal tabungan</p>
          </div>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="p-5 space-y-4">
          <div>
            <label className="text-sm font-semibold">Saving Goal</label>
            <input
              type="number"
              min={1}
              value={tabung}
